...
> *(Qwiklabs Assessment 1 / 50 pts: all three tables in the BigQuery `enterprise_finops_gold` dataset hold 100 rows in total)*
...
> 🔎 **Self-check**
> This task is about reading and understanding source code, which leaves nothing in the cloud for a
> grader to observe, so it is not a scored checkpoint.
> Instead, confirm that `.venv/bin/python3 scripts/validate_agent.py` reports **10 PASSED, 0 FAILED**.
...
> 🔎 **Self-check**
> A locally running `uvicorn` process is not visible to any cloud API, so this is not a scored checkpoint.
> Instead, confirm that `execution_engine` in the `/healthz?deep=true` response reads
> `ADK_2.0_RUNNER (gemini-2.5-flash)`.
...
> ✅ **Check my progress**
> **Click Check my progress to verify the objective.**
> *(Qwiklabs Assessment 2 / 50 pts: the Cloud Run service `enterprise-hub-agent` is `Ready=True` and has `GOOGLE_GENAI_USE_VERTEXAI=TRUE` set)*
...
> 🔎 **Self-check**
> Registration happens in the Gemini Enterprise console, which a grading script cannot query, and the
> Discovery Engine `authorizations` API is currently ALPHA-restricted. This is not a scored checkpoint.
> Instead, confirm that `.venv/bin/python3 scripts/verify_lab04_completion.py` reports **`[PASS]` on all four checks**.
